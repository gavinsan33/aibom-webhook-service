...
`source_code.git_commit`/`git_repository`/`git_branch` are filled in from three sources, in order of precedence:

1. **Annotation** — `aibom.io/git-repository`/`git-commit`/`git-branch`, if set.
2. **OpenShift BuildConfig commit labels** — if no annotation is set, `detect_git_provenance_from_containers` (`postprocess.py`) looks up the container's image by digest (`containers.json`'s `image_id`, populated in the watcher from `ContainerStatuses[].ImageID` — the resolved digest, not the mutable tag a re-push could spoof) against the cluster-scoped `image.openshift.io` `Image` object, and reads whatever `io.openshift.build.commit.id`/`.ref`/`.source-location` labels OpenShift's build controller baked onto it.
3. **OCI-standard image labels** — if no OpenShift-specific labels are present, falls back to the vendor-neutral `org.opencontainers.image.revision`/`.source` labels on the same `Image` object's config. These are set by tooling other than an OpenShift `BuildConfig` (GitHub Actions' `docker/metadata-action`, `docker buildx build --label`, Cloud Native Buildpacks, `ko`, Jib, ...), so this extends detection to images built by any CI system that follows the OCI convention — not just in-cluster builds. There's no standard OCI label for the branch, so this tier only ever yields commit + repository, never `git_branch`.

If a container's image has neither set of labels (a prebuilt image with no CI-stamped provenance at all — most public base images), this falls through to nothing. RBAC for the underlying `Image` lookup (`aibom-postprocess-images` ClusterRole, bound per-namespace) is intentionally narrow — `get` on `images` only, nothing else in `image.openshift.io`.

Whichever source wins is recorded in `source_code.declared_via` (`"annotation"`, `"openshift_build_label"`, or `"oci_image_label"`), mirroring `dataset.declared.declared_via`.

**This is identification, not verification, and it isn't a security/compliance artifact.** The build labels record what the build controller believes it cloned — not that nothing was injected into the image afterward (e.g. an extra `RUN` step after the `git clone`), and not that the source was vetted. Digest-keying only protects against a re-tagged image claiming someone else's labels; it does nothing about a workload that simply never went through a `BuildConfig` in the first place (a prebuilt image pulled from a public registry lands with no signal here at all, same as if nothing were detected). Reaching genuine tamper-evident provenance would require a signed attestation (e.g. Tekton Chains/SLSA, verifiable independent of trusting the live cluster) plus admission-time enforcement rejecting unsigned images — both out of scope here; this only records what's cheaply available and labels its source honestly.

Other provenance sources (in-container `.git` reads, `pip install git+...`'s PEP 610 `direct_url.json`, Tekton `git-clone` task results) aren't implemented — see the project's git-provenance design discussion for the fuller set of options and their tradeoffs.
