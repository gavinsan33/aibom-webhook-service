...
`isPostprocessPod` additionally requires a plain `batch/v1` `Job` owner reference before honoring the label at all — real postprocess pods are always Job-owned, so a raw Pod submitted directly (never wrapped in a Job) can't be one. This is defense-in-depth, not a full close: Kubernetes doesn't validate that a submitted object's `ownerReferences` point to anything real, so a requester who fabricates a Job-kind owner reference on their own raw Pod alongside the label still gets through undetected by either check above (since no Job was ever created for the Job-admission check to see). Fully closing that would need comparing this admission's own `request.userInfo` against the Job controller's identity, which isn't implemented.
