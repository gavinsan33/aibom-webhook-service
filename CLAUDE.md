...
`source_code.git_commit`/`git_repository`/`git_branch` are filled in from two sources, in order of precedence:

1. **Annotation** — `aibom.io/git-repository`/`git-commit`/`git-branch`, if set.
2. **OpenShift BuildConfig commit labels** — if no annotation is set, `detect_git_provenance_from_containers` (`postprocess.py`) looks up the container's image by digest (`containers.json`'s `image_id`, populated in the watcher from `ContainerStatuses[].ImageID` — the resolved digest, not the mutable tag a re-push could spoof) against the cluster-scoped `image.openshift.io` `Image` object, and reads whatever `io.openshift.build.commit.id`/`.ref`/`.source-location` labels OpenShift's build controller baked onto it. This only resolves anything if the image was actually built in-cluster via a `BuildConfig`; a prebuilt or externally-pushed image has no such labels and this falls through to nothing. RBAC for this (`aibom-postprocess-images` ClusterRole, bound per-namespace) is intentionally narrow — `get` on `images` only, nothing else in `image.openshift.io`.

Whichever source wins is recorded in `source_code.declared_via` (`"annotation"` or `"openshift_build_label"`), mirroring `dataset.declared.declared_via`.
